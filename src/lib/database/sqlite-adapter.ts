// SQLite adapter using sql.js (SQLite compiled to WASM).
// Data persists in IndexedDB — no internet connection required.
// Mimics the Supabase PostgREST query-builder + auth + realtime API.
//
// CRITICAL FIX: Every mutation (insert/update/upsert/delete) now calls
// persist() to save the database to IndexedDB. Without this, data was lost
// on every page reload.

import { SCHEMA_SQL, SEED_SQL, uuid } from './sqlite-schema';
import { dbLog, logQuery, logAuth, logInit } from './logger';
...
class QueryBuilder<T = Row> {
  ...
  constructor(db: Database, persist: () => Promise<void>, table: string) {
    this._db = db;
  ...
  eq(column: string, value: unknown): this {
    ...
    this._whereParams.push(this._encode(column, value));
    return this;
  }
  ...
  in(column: string, values: unknown[]): this {
    ...
    for (const v of values) this._whereParams.push(this._encode(column, v));
    return this;
  }

  is(column: string, value: unknown): this {
    if (value === null) this._where.push(`${column} IS NULL`);
    else { this._where.push(`${column} = ?`); this._whereParams.push(this._encode(column, value)); }
    return this;
  }
  ...
  private _encode(column: string, val: unknown): string | null {
    if (val === null || val === undefined) return null;
    // JSON columns: store as JSON so booleans/numbers/arrays round-trip correctly
    if (JSON_COLUMNS.has(column)) return JSON.stringify(val);
    // Boolean columns: store as 1/0
    if (typeof val === 'boolean') return val ? '1' : '0';
    // Objects/arrays in non-JSON columns: stringify
  ...
  private async _execute(): Promise<{ data: unknown; error: PostgrestError | null }> {
    ...
    let opType = 'select';
    if (this._insertData) opType = this._isUpsert ? 'upsert' : 'insert';
    else if (this._updateData) opType = 'update';
    else if (this._isDelete) opType = 'delete';

    try {
      let result: { data: unknown; error: PostgrestError | null };
      if (this._insertData) result = await this._execInsert();
      else if (this._updateData) result = await this._execUpdate();
      else if (this._isDelete) result = await this._execDelete();
      else result = this._execSelect();

      const elapsed = Date.now() - startTime;
      if (result.error) {
        logQuery(this._table, opType, elapsed, result.error.message);
      } else {
        logQuery(this._table, opType, elapsed);
      }
      return result;
    } catch (e: any) {
      const elapsed = Date.now() - startTime;
      const msg = e?.message ?? 'Database error';
      logQuery(this._table, opType, elapsed, msg);
      dbLog('error', opType, `Exception on "${this._table}": ${msg}`, {
        table: this._table,
        where: this._where,
        error: e,
      });
  ...
  private async _execInsert(): Promise<{ data: unknown; error: PostgrestError | null }> {
    ...
    for (const input of dataArr) {
      ...
      if (this._isUpsert) {
        ...
        this._db.run(sql, cols.map((c) => this._encode(c, row[c])));
      } else {
    ...
    // CRITICAL FIX: persist after every mutation
    await this._persist();
  ...
  private async _execUpdate(): Promise<{ data: any; error: PostgrestError | null }> {
    ...
    const params = cols.map((c) => this._encode(c, updateData[c]));
  ...
  private async _execDelete(): Promise<{ data: any; error: PostgrestError | null }> {
...
async function createSqliteAuth(db: Database, persist: () => Promise<void>) {
  ...
  function notify(event: string, session: any) {
    for (const l of listeners) {
      try { l(event, session); } catch (e) { dbLog('error', 'auth.notify', 'Listener error', { error: e }); }
    }
  }
  ...
  async function signInWithPassword({ email, password }: { email: string; password: string }) {
    logInit(`signIn attempt for "${email}"`);
    const stmt = db.prepare('SELECT * FROM auth_users WHERE email = ? LIMIT 1');
    stmt.bind([email]);
    if (!stmt.step()) {
      stmt.free();
      logAuth('signIn', false, 'User not found');
    ...
    if (!valid) {
      logAuth('signIn', false, 'Password mismatch');
      return { data: { user: null, session: null }, error: { message: 'Invalid login credentials' } };
    }
  ...
  async function signUp({ email, password, options }: { email: string; password: string; options?: { data?: Record<string, any> } }) {
    ...
    if (checkStmt.step()) {
      checkStmt.free();
      logAuth('signUp', false, 'Email already registered');
...
class SqliteClient {
  ...
  from(table: string): QueryBuilder { return new QueryBuilder(this.db, this.persistFn, table); }
...
export async function createSqliteClient(): Promise<SqliteClient> {
  logInit('Initializing SQLite (offline mode)...');
  try {
    const sql = await loadSqlJs();
    logInit('SQLite WASM loaded');
  } catch (e) {
    dbLog('error', 'init', 'Failed to load SQLite WASM', { error: e });
    throw e;
  }
  ...
  if (savedData) {
    ...
    logInit('Loaded existing SQLite database from IndexedDB');
  } else {
    db = new (SQL!).Database();
    db.run(SCHEMA_SQL);
    db.run(SEED_SQL);
    await saveToIDB(db.export());
    logInit('Created new SQLite database with schema + seed data');
  }
  ...
  const persist = async () => {
    try {
      ...
      dbLog('error', 'persist', 'Failed to persist database to IndexedDB', { error: e });
    }
  };

  const client = new SqliteClient(db, persist);
  await client.initAuth();
  logInit('SQLite client ready');
  return client;
}
