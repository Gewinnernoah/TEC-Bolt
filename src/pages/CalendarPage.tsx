...
export function CalendarPage() {
  ...
  const allEvents = useMemo(() => {
    ...
    (loans ?? []).forEach((l) => {
      ...
      events.push({ date: l.expected_return_at, title: `Rueckgabe faellig: ${l.teacher?.full_name ?? 'Unbekannt'}`, type: 'loan', room: l.room?.name });
    });
    (requests ?? []).forEach((r) => {
      if (r.pickup_at) events.push({ date: r.pickup_at, title: `Abholung: ${r.teacher?.full_name ?? 'Unbekannt'}`, type: 'request', room: r.room?.name });
      if (r.return_at) events.push({ date: r.return_at, title: `Rueckgabe: ${r.teacher?.full_name ?? 'Unbekannt'}`, type: 'request', room: r.room?.name });
    });
    return events;
  }, [loans, requests]);
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Verfuegbarkeitskalender" subtitle="Echtzeit-Ueberblick ueber Ausleihen und Reservierungen" actions={
      ...
      <div className="card p-4">
        <h3 className="mb-3 text-sm font-semibold text-slate-200">Raum-Verfuegbarkeit heute</h3>
